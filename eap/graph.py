...
class Graph:
    ...
    def apply_topn(self, n: int, absolute: bool):
        ...
        # 处理 edge.score 为 None 的情况，若为 None 则视为 0
        sorted_edges = sorted(list(self.edges.values()),
                            key=lambda edge: a(edge.score) if edge.score is not None else 0,
                            reverse=True)
    ...
    #         1. 'cfg': Configuration dictionary.
    #         2. 'nodes': Dict[str, bool] mapping node name to a boolean indicating if the node is part of the circuit.
    #         3. 'edges': Dict[str, Dict] mapping an edge name to a dictionary containing at least 'score' and 'in_graph'.
    #         """
    #         with open(filename, 'r') as f:
    #             d = json.load(f)
    #         g = cls.from_model(d['cfg'])
            
    #         # 设置节点状态
    #         for name, in_graph in d['nodes'].items():
    #             g.nodes[name].in_graph = in_graph
            
    #         # 设置边状态；如果边不存在则新建一个 Edge 对象
    #         for name, info in d['edges'].items():
    #             if name in g.edges:
    #                 g.edges[name].score = info['score']
    #                 g.edges[name].in_graph = info['in_graph']
    #             else:
    #                 g.edges[name] = Edge(name=name, score=info['score'], in_graph=info['in_graph'])
